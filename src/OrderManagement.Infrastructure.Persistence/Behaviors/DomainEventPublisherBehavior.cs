...
namespace OrderManagement.Infrastructure.Persistence.Behaviors;
...
public sealed class DomainEventPublisherBehavior<TRequest, TResponse> : IPipelineBehavior<TRequest, TResponse> where TRequest : ICommand
{
    ...
    public DomainEventPublisherBehavior(Channel<IDomainEvent> channel, SqlServerDbContext context)
    {
        _channel = channel;
        _context = context;
    }
